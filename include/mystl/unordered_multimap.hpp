...
namespace mystl 
{
    ...
    private:
        ...
        void adopt_or_move(UnorderedMultiMap& other)
        {
            bool take_ownership;
    ...
    public:
        ...
        public:
            ...
            using value_type        = UnorderedMultiMap::value_type;
            using difference_type   = UnorderedMultiMap::difference_type;
            using pointer           = UnorderedMultiMap::pointer;
            using reference         = UnorderedMultiMap::reference;
        ...
        public:
            ...
            using value_type        = UnorderedMultiMap::value_type;
            using difference_type   = UnorderedMultiMap::difference_type;
            using pointer           = UnorderedMultiMap::const_pointer;
            using reference         = UnorderedMultiMap::const_reference;
        ...
        public:
            ...
            ConstIterator(const Iterator& it) : node_(it.node_), bucket_idx_(it.bucket_idx_), map_(it.map_) {}
    ...
    public:
        ...
        UnorderedMultiMap(const UnorderedMultiMap& other, const allocator_type& alloc)
            : bucket_count_(other.bucket_count_)
            , max_load_factor_(other.max_load_factor_)
            , node_alloc_(alloc)
            , hash_func_(other.hash_func_)
        ...
        UnorderedMultiMap(UnorderedMultiMap&& other, const allocator_type& alloc)
            : bucket_count_(other.bucket_count_)
            , max_load_factor_(other.max_load_factor_)
            , node_alloc_(alloc)
            , bucket_alloc_(alloc)
        ...
        iterator find(const Key& key) 
        {
            ...
            while (curr) 
            {
                if (equal_func_(curr->value.first, key)) return iterator(curr, idx, this);
                curr = curr->next;
            }
            return end();
        }
        ...
        const_iterator find(const Key& key) const 
        {
            ...
            while (curr) 
            {
                if (equal_func_(curr->value.first, key)) return const_iterator(curr, idx, this);
                curr = curr->next;
            }
            return cend();
        }
        ...
        size_type count(const Key& key) const 
        {
            ...
            while (curr) 
            {
                if (equal_func_(curr->value.first, key)) cnt++;
                else if (cnt > 0) break;
                curr = curr->next;
            }
            return cnt;
        ...
        mystl::Pair<iterator, iterator> equal_range(const Key& key) 
        {
            ...
            while (curr) 
            {
                ...
                {
                    Node* first = curr;
                    while (curr && equal_func_(curr->value.first, key)) curr = curr->next;
        ...
        {
            // Rehash before creating a node for strict exception safety
